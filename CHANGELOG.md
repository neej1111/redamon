...
- **Installing on ARM64 (Apple Silicon, ARM Linux) no longer fails in the TruffleHog image.** The Dockerfile always downloaded the `linux_amd64` release; its checksum verified, then the smoke test died with `Exec format error`. The asset now follows the build architecture. The agent image had the same bug in its Go toolchain, where it was silent: the build passed and Go failed only when the agent first ran it. That download now follows the architecture too, with a per-architecture pinned sha256 and a `go version` check at build time. A static test fails if any Dockerfile names an x86 download again.
- **`sca-intel-sync` no longer leaves a fresh install with an empty incident catalog while supplychainattack.org is down.** The feed currently answers `HTTP 402` (Vercel `DEPLOYMENT_DISABLED`). A sync that cannot use the feed now installs a bundled, indicators-only copy of the catalog (2026-08-18 snapshot, ~115 KB, sha256-pinned and re-validated on load) when the volume holds nothing usable, and says so; a catalog already on the volume is kept and never overwritten by the copy. The live feed is retried on the normal retry floor and replaces the copy once it answers. The retry floor no longer delays the copy (it spares the feed, and the copy needs no fetch), and air-gapped deploys (`SCA_INTEL_AUTO_REFRESH=false`) now receive it too, through `./redamon.sh sca-intel-sync --seed-only`, which runs with no network.
- **A feed outage reads as one.** The sync names the HTTP status and whose side it is on (`HTTP 402 (the feed's hosting is paused or disabled upstream) [DEPLOYMENT_DISABLED]`), reports the catalog revision it kept, and `redamon.sh` prints a warning instead of an error, because RedAmon runs normally without a refresh.
- **The agent's WebSocket API no longer calls `datetime.utcnow()`**, deprecated since Python 3.12 and scheduled for removal. It uses `datetime.now(timezone.utc)` with the timezone stripped, so every stored timestamp and every timestamp sent to the webapp is byte-for-byte what it was. No change on the current Python 3.11 image; it keeps the agent working on 3.12+. Contributed by @MBK-fr in #187.
...
- **A hidden virtual host no longer claims to be a DNS record.** Finding that a server answers for a candidate `Host` header or SNI name proved only how it routes, but the graph recorded a `RESOLVES_TO` edge as though a resolver had returned that address. Behind a shared frontend or CDN the two routinely disagree, so a name that really resolves elsewhere gained a second, false answer. The probe no longer writes that edge, and re-probing a pair removes a leftover one that no DNS writer ever corroborated (an edge carrying `record_type` is left alone). Reported by @xdCloudy in #189.
- **Findings for hidden vhosts stay reachable.** A hidden panel is usually in no zone file at all, which is the point of looking for it, so dropping the DNS claim must not drop the finding with it. A candidate under the engagement's domain still gets its `Subdomain`, now flagged `has_dns_records: false` until a resolver confirms it, and keeps the documented "hidden admin panels" traversal working. A co-hosted third-party name is no longer added to the inventory at all: its finding, and any BaseURL discovered for it, hang off the `IP` and `Service` that served them. A lookalike domain (`notexample.com` against `example.com`) is no longer mistaken for a child of the target.
- **JS recon no longer files third-party URLs as target endpoints.** Every URL a bundle names used to become an `Endpoint`: a payment API, a CDN, a partner host. It is now checked against the scan scope as it is written, with the same host set the crawler and Nuclei writers use, plus the subdomains JS recon itself discovered and vetted. Out-of-scope hosts are counted in `endpoints_out_of_scope`. Reported by @xdCloudy in #190.
- **Third-party hosts named in JavaScript are recorded as `ExternalDomain` nodes** (source `js_recon`) by the full pipeline. JS recon collected them but the external-domain aggregate never read its list, so they reached the graph only as bogus Endpoints. In IP mode the target IPs, which JS recon also lists as external, are left out.
...
- The VHost & SNI entry in the graph schema states how a finding is attached and that the source writes no `RESOLVES_TO`, so the agent knows to reach a co-hosted finding from the `IP`. A `vhost_target` guinea pig (three virtual hosts on one address, none of them in DNS) backs it with an end-to-end test that drives the real module and asserts what the graph ends up saying.
...
- **RedAmon as an MCP server: let your own AI agent drive recon.** Until now RedAmon was always the MCP *client*. It can now be the *server*: an external agent connects in with a personal access token, authenticates as one RedAmon user, and works inside that user's own projects. **Thirty tools across nine permissions** — read projects, scans, settings and findings; summarise, query and diff the attack-surface graph; start, stop and queue a recon; record a triage verdict; run a single scope-checked command in the Kali sandbox. Off by default; mint a token in Global Settings → **MCP Server**, which sits beside the existing MCP Tool Plugins tab (outbound). See [README.MCP.SERVER.md](docs/readmes/README.MCP.SERVER.md).
  ...
  **What it can change is a positive allowlist, not a promise.** `Project` has over 700 columns and the settings endpoint writes whatever it is given, so the MCP path enumerates ~126 genuine tuning fields — per-tool toggles, rate limits, thread counts, timeouts, depth caps, severity lists — each numeric bounded by the same min/max as its form input. Everything else is denied by class and by name: the engagement target and scope, Rules of Engagement, which container images get spawned, other scans' targets, egress toggles, wordlists and templates, request headers, intrusiveness toggles, credentials, agent settings. A test asserts every column is classified, so a new field is unreachable over MCP until someone classifies it. `Project` columns now carry a **read** classification as well as a write one — they genuinely differ, and among the Rules of Engagement fields are a client's name, email and phone number.

- **The surface can observe, not only act.** It shipped holding the most consequential verb in the product — `start_recon`, which rebuilds the graph and consumes a retention slot — while being unable to answer what was found, what changed since last time, or what is happening right now. Fourteen reads close those gaps.

  **Findings and fixes.** `list_findings` refuses to imply a ranking it does not have: `triage_priority_score` is written only by a triage run someone starts deliberately, so it reports `triageState` rather than letting a caller read *unscored* as *unimportant*. `list_muted_findings` closes a false negative that was invisible by construction — muted findings are excluded from `graph_summary`, from `list_findings` and from Cypher, so "zero open findings" and "a human suppressed thirty criticals" were the same answer. Both, with `list_remediations`, sit behind a **new `triage:read`** permission that is off by default: folding them into `recon:read` would have changed what every already-minted credential can read on deploy day.

  **State and history.** `get_project_activity`, `list_scan_versions`, `compare_scan_versions` and `get_scan_status`, the last covering the six scanners outside the recon pipeline. `compare_scan_versions` summarises rather than handing back the raw delta, whose identity keys embed secret *values*, and refuses a comparison that straddles a version activation instead of reporting the whole project as removed.

  **Catalogues.** `describe_recon_settings` makes `update_recon_settings` usable: it was a 126-field API whose only reference was a list of key names, so an agent learned each bound by being refused — and one bad key refuses the whole call, so a batch of guesses applied nothing. `list_recon_presets` is read-only deliberately: between a third and 60% of each preset is outside what this surface may write, and for the stealth presets the excluded part *is* the stealth, so applying one here would leave a scan louder than asked for while reporting success.

  **Three analytics views** an agent otherwise reconstructs badly from natural language: `get_attack_surface_overview`, `list_exploit_paths` and `get_blast_radius`. Their Cypher was written fresh rather than lifted from the in-app views, and two of those views turn out to be quietly wrong: a bare re-reference to a CVE node loses its global-reference exemption and gets tenant properties injected into a node type that carries none, so it matches nothing. The Red Zone kill-chain view reports `cisaKev` false for every row and the blast-radius view sorts by a `kevCount` that is permanently zero. No error, no empty result, the view renders — the ranking is simply gone.

  **Saved graph views** become runnable by name. `list_graph_views` deliberately never returns their query text; `run_graph_view` requires the same permission as raw Cypher, because choosing *which* stored query runs is enough, especially since the query is not validated when it is saved.

- **A queue instead of a dead end, and a verdict that persists.** `queue_recon` turns a busy project from a refusal into a plan, behind its own **`recon:queue`** permission because a queued job dispatches *later* and outlives the token that created it — `JobQueue` records no token, so revoking the credential does not cancel its work. `cancel_queued_scan` reads the update count, so a job that started during the race is reported as *not cancelled* rather than a false success.

  `set_finding_verdict` marks a finding confirmed, likely noise, or back to unreviewed, behind a new **`triage:write`**. It records the verdict as the operator's own rather than inventing a third provenance value: `triage_source` is a closed two-value set that four behaviours branch on, and a third value would make the finding **prune-eligible**, let a later AI run overwrite it, stop `likely_noise` meaning false-positive, and render it as "Not reviewed". The channel is recorded on its own property instead. It refuses mid-triage-run and fails closed if it cannot tell. **It cannot mute or unmute, in either direction** — "this is a false positive, mute it" is an entirely plausible instruction to find embedded in scanner output, and the invariant that only a person mutes is exactly what bounds a prompt injection to *mislabelling a verdict a human can overrule*.

- **Agent Onboarding: the pack that teaches an external agent this surface.** Thirty tools and nine permissions are useless to an agent that does not know which to reach for, and a README is the wrong artefact — it is not in the agent's context when it matters. Picking an **Agent Profile** at mint time now does two things: it suggests the permission set for that job, and it generates the onboarding document.

  Fourteen profiles, from bug bounty and pentest through attack-surface monitoring, triage, CI gating, compliance, SOC enrichment and M&A risk. **A profile is never an authorization input.** `requireScope` and every tool body read `scopes` and nothing else; a token's power is exactly its ticked permissions whatever its profile says. A test scans `mcpAuth.ts` for the word *profile* and fails if it appears, because a second, weaker authorization path is the one genuinely dangerous thing this feature could introduce. **No profile ever auto-ticks `kali:exec` or `recon:overwrite`** — one reaches a live target outside a scan, the other destroys a graph irreversibly, and neither may arrive as a side effect of choosing from a dropdown. Both are offered *unchecked* instead.

  Two shapes from one source: a `SKILL.md` plus reference files for clients that read them, and a bounded inline `instructions` string returned at `initialize` for every other client. The scope filter is hard — a tool the token cannot call appears only in the "cannot do" tail, with the permission it would need — while the profile filter is only editorial. The pack is generated from the live tool list, so a tool withdrawn on this deployment never appears in it.

- **Give your agent a shell at your target.** `kali_exec` runs one command in RedAmon's Kali sandbox, `kali_output` pages its output from a byte cursor, `kali_cancel` stops it, and `kali_toolbox` describes what is installed.

  **It is a shell, and that is a deliberate choice rather than an oversight.** An earlier form of this permission admitted 23 binaries through a per-flag allowlist with every named host checked against the project's scope. That guard is gone: `kali_exec` is now `bash -c` with the sandbox's full toolset, pipelines and redirection, no allowlist and no per-command target check. An agent holding `kali:exec` can run any installed binary against any host the sandbox can reach.

  **What that leaves is stated plainly rather than left to inference.** The sandbox's own environment carries `SCANNER_API_KEY`, `MCP_AUTH_TOKEN` and `TUNNEL_AUTH_TOKEN`, so `env` is a credential read — and `SCANNER_API_KEY` is the one to weigh, because this route deliberately refuses `X-Scanner-Key` as an *inbound* credential on the grounds that the scanner tier is least trusted. An agent that reads it out of the sandbox has recovered what the route declined to accept. On a cloud deploy the instance metadata endpoint is reachable, which on AWS returns the instance role's credentials.

  **So the containment is now entirely in who gets the scope**, and it is deliberately hard to grant by accident. **Four switches must all be on**, each owned by a different person: `MCP_KALI_EXEC_ENABLED` on the deployment, the `kali:exec` permission on the token (password-confirmed at mint, and never auto-ticked by any Agent Profile), **Allow MCP Sandbox Commands** on the project — a human, per engagement, and denied to `update_recon_settings` by name so a token can never grant itself this — and a configured target. Every call is audited, and with no refusal path left **the audit row is the only record of what an agent did**. See [README.MCP.SERVER.md](docs/readmes/README.MCP.SERVER.md), "The `kali:exec` residual, stated plainly".

- **graph_summary and graph_schema, for RedAmon's own agent too.** Same three graph tools on both surfaces. `graph_summary` answers "what does this project actually contain" as a count per node type, which is how an agent tells *scanned and clean* from *never scanned* — reporting the second as the first is a false negative in a security tool. `graph_schema` explains what the graph means, reading no data, so it still answers when a query does not.

- **Operator levers.** A single misbehaving tool can be withdrawn with `MCP_DISABLED_TOOLS` without taking the surface down; withdrawn tools disappear from the advertised list rather than staying visible and refusing, and a name matching no tool is ignored, because an emergency lever must not be the reason the server fails to start. Audit rows now carry a call's filters and its result count, so `list_findings / project:abc / ok` no longer reads the same for a token that pulled every finding and one that pulled none. `get_recon_status` reports the total phase count and the domain-batch group, because phases restart per group and on a multi-domain scan the phase number barely moves for an hour.
...
- **Nine defects found by reviewing the new tools as failure modes rather than as code.** Every one produced a wrong answer rather than an error, and each now has a test that fails against the old behaviour.

  **Two silently shrank a security answer.** `list_exploit_paths` and `get_blast_radius` filtered superseded exploit records with a row-level predicate instead of an inner join condition, so a CVE whose only exploit record had been superseded vanished from the ranking entirely. `list_findings` applied a filter to the highest-ranked 2,000 findings and reported the matches as a complete total, so a large project could answer "four criticals" when there were forty; the count is now explicitly a floor when the window was full.

  **One did the wrong thing on request.** `compare_scan_versions` accepted `from: "current"`, documented it, then silently substituted a stored version. **One crossed a tenant boundary:** `queue_recon` and `compare_scan_versions` charged their per-project budget before checking ownership, so anyone holding a project id could exhaust another tenant's scan-start window — and its owner was refused with a message blaming their own token.

  **Three were the "never report a dependency failure as an empty result" rule not actually applied**, in the activity helper, `get_scan_status` and `list_muted_findings`; all three now report unknown, and the muted cap travels with the request instead of pulling every suppressed finding across the wire. **Two were operability:** paging past the 2,000-finding window returned empty pages while still flagging "there is more", and a deploy that rebuilt the webapp without the agent silently lost both the graph concurrency ceiling and verdict provenance, because the older agent ignores fields it does not know and still answers 200.

- **Twelve more found by driving the finished surface as an external agent**, across two parallel end-to-end exercises — one on `kali_exec`, one across all thirty tools. **Not one was a layer being wrong.** In every case a component reported its own state correctly and the *composed* answer misled the caller, which is why the full gate passed before and after every fix.

  **Four made a failure look like a result.** `get_recon_status` passed through the orchestrator's *live* state, which is a flat `idle` once the container exits — identical whether a scan completed, failed, was canceled at 10%, or never ran; `ScanJob` held the durable record and nothing consulted it. `stop_recon` returned that same post-stop `idle` either way, so a stop that halted nothing was indistinguishable from one that worked, and the audit row said `ok` for both. A Kali job that **ran and failed** was reported as 404 "no such command", because both endpoints read any `error` key as a lookup miss — so a scan killed at the 300s cap had its output on disk, unreachable, while the caller was told the job never existed. And a timed-out scan now says how to split the work, because a killed scan reading as a clean one is a false negative.

  **Three threw away the reason.** `graphClient` quoted the agent's error only when the body carried a `stage` field, which only the natural-language path sets — so every refusal the graph layer writes was replaced by a flat "graph query failed" on the one path where the caller composed the query and could have fixed it. The same shape in `kaliClient` swallowed the 409 that says why a job could not be cancelled. And the child-ownership helpers reported a missing `versionId`, `jobId` or `viewId` as "Project not found", one line after the project check had already passed.

  **Two taught the agent something untrue.** `kali_toolbox` served the in-app agent's full bash catalogue, advertising sqlmap, msfvenom and nc to a caller refused on every one; it now leads with what `kali_exec` actually admits, generated from the guard's own spec table. `list_exploit_paths` and `get_blast_radius` both start from a CVE relationship, so a project with no CVE enrichment answered `returned: 0` however much it held, with nothing saying why.

  **Three were the tests' own blind spots.** The fix for the toolbox was routed to the wrong function — a helper landed between the decorator and the handler, so the endpoint returned a bare string while every test in the file passed, because they all call the handler directly and never touch the routing. The live binary check probed with `bash -lc` where the sandbox runs `bash -c`, and the login profile drops `/root/go/bin`, so it called ten installed tools missing. And `load_project_settings` swallows every fetch error and falls back to defaults whose target scope is **empty**, so an unreachable webapp read as "this project has no target configured" on projects that were configured correctly — safe only because that default happens to be empty.

- **`kali_exec` could not be switched on at all on a deployed host.** `MCP_KALI_EXEC_ENABLED` and `MCP_RATE_EXEC_PER_MIN` were wired into the compose file but never seeded into the deployed application's `.env`, so compose interpolated them to empty and fell back to its own `false` default. An operator who set the switch in their deploy `.env` got no error and no effect. The repo's own env-chain test was already failing on both; it now passes, and it catches the next one.

- **`graph_summary` called the graph settled during five of the seven scan kinds.** It resolved `liveGraphState` from a check covering full and partial recon only, while GVM, GitHub Secret Hunt, TruffleHog, supply-chain and AI attack-surface scans all write finding nodes into the same live graph. An agent asking for a malicious-package count *during the supply-chain scan that produces those nodes* got a partial count stamped `stable`, which the manual documents as "the counts are trustworthy" — the same false negative the tool exists to prevent, absent in exactly the window it was built for. It now reads all seven kinds in **one** orchestrator call, adds `agent_writing` and `unknown`, and ranks `unknown` above every running state because the one thing it must never be mistaken for is `stable`. Another project's running scan is filtered out before anything is returned.

  It also silently hid resolved findings from its own counts: the census leaves out findings a later scan stopped reporting, while raw Cypher includes them. The answer now carries `hiddenFromCounts.stale`, and when that figure cannot be read the **key is absent rather than `0`**, because a fabricated zero would be the same false negative the field exists to close.

- **`get_project_activity` moves from the read bucket to the question bucket.** One call costs up to eight requests to the scan orchestrator every project on the host shares, and telling the model not to poll is advice to an untrusted caller rather than a control.

### Security

- **`kali:exec` grants a shell, and the permission copy now says so.** The scope was described to operators as "a fixed allowlist of read-only tools" whose every command was "checked against the project's own scope". That was true of an earlier build and is not true of what ships: the admission guard and its regression tests were removed in favour of a real shell. The consent screen, the generated API reference and the security manual were rewritten together rather than separately, because a permission whose description overstates its containment is worse than one that is plainly dangerous — an operator ticks it believing a check exists.

  Along the way an adversarial review of the *old* guard had broken it ten ways — `curl --resolve=target:443:6.6.6.6`, an attached short value (`-xevil.tld`), a trailing slash, `/workspace/../etc/...`, `whatweb --plugins` (arbitrary Ruby), `nikto -config`, `dig @evil.tld`, `curl -K` — all of which reached outside the engagement. That work is superseded: there is no host check left to bypass. It is recorded here because it is the reason the surface is now honest about being a shell instead of claiming a boundary it kept failing to hold.
...
- **Graph reads are bounded.** Every guard on the graph endpoint was about *what* may be read; none bounded *how much*. One read-only Cartesian product passed them all and pinned Neo4j, which the graph screen, the agent and every running scan share. Reads now carry a transaction timeout, a streamed record cap that reports truncation, and a serialised-byte cap that refuses rather than truncating silently. None of them rewrites the caller's query.
